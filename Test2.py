...
def sort_columns(matrix):
    ...
    # transpose matrix, so that rows become columns
    for i in range(len(matrix)):
    ...
    for i in range(len(transposed_matrix)):
        transposed_matrix[i].sort(reverse=(i % 2 == 0))
...
pretty_print_matrix(sort_columns(matrix_random))